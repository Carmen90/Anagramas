...
void Telefonos (){
	...
	while ( !cin.eof()){
		...
		while ( i < palabra.length()){
			...
			else if (palabra[i] == 'D' || palabra[i] == 'E' || palabra [i] == 'F'){
				telefono += 3;
				telefono *= 10;
				i++;
			}
			else if (palabra[i] == 'G' || palabra[i] == 'H' || palabra [i] == 'I'){
				telefono += 4;
				telefono *= 10;
				i++;
			}
			else if (palabra[i] == 'J' || palabra[i] == 'K' || palabra [i] == 'L'){
				telefono += 5;
				telefono *= 10;
				i++;
			}
			else if (palabra[i] == 'M' || palabra[i] == 'N' || palabra [i] == 'O'){
				telefono += 6;
				telefono *= 10;
				i++;
			}
			else if (palabra[i] == 'P' || palabra[i] == 'R' || palabra [i] == 'S'){
				telefono += 7;
				telefono *= 10;
				i++;
			}
			else if (palabra[i] == 'T' || palabra[i] == 'U' || palabra [i] == 'V'){
				telefono += 8;
				telefono *= 10;
				i++;
			}
			else if (palabra[i] == 'W' || palabra[i] == 'X' || palabra [i] == 'Y'){
				telefono += 9;
				telefono *= 10;
				i++;
			}
			else if (palabra[i] >= '0' && palabra [i] <= '9'){
				telefono += palabra[i]- '0';
				telefono *= 10;
				i++;
			}else{
				i++;
		...
		if (arbol.esta(telefono)){
			arbol.inserta (telefono, arbol.consulta(telefono) + 1);
		}
		else{
			arbol.inserta( telefono, 1);
		}


	}

	Arbus <int, int> ::Iterador it = arbol.principio();

	while ( it != arbol.final()){
		if ( arbol.consulta(it.clave()) > 1){

			cout << it.clave()/ 10000 << "-" << it.clave()%10000;
			cout << " " << arbol.consulta( it.clave()) << endl;
