...
void Ananagrams (){
	...
	while ( palabra != "#"){
		...
		Arbus <string, int>::Iterador it = arbContador.principio();

		while ( it != arbContador.final()){
		if ( it.valor() == 1){
			orden.inserta (arbol.consulta(it.clave()), arbol.consulta(it.clave()));
			
		}
		it.avanza();
	}
		Arbus <string, string> ::Iterador itOrden = orden.principio();

		while ( itOrden != orden.final()){
			cout << orden.consulta(itOrden.clave()) << endl;
			itOrden.avanza();
		}
